...
<<<<<<< HEAD
- `grid_price_risk` (ramp-based forecast price bias using `ramp_start_after_minutes` + `ramp_duration_minutes`, with optional `import_price_floor`/`export_price_ceiling` clamps applied before scaling)
