...
<<<<<<< HEAD
  - Forecast price risk bias (`plant.grid.grid_price_risk`) scales import/export prices over the horizon using `ramp_start_after_minutes` + `ramp_duration_minutes`; optional `import_price_floor`/`export_price_ceiling` clamp raw prices before scaling.
  - `plant.grid.zero_price_export_preference` controls whether the zero-price bonus favors export or curtailment.
=======
  - Forecast price risk bias (`plant.grid.grid_price_risk`) scales import/export prices over the horizon using `ramp_start_after_minutes` + `ramp_duration_minutes`; optional `import_price_floor`/`export_price_ceiling` clamp raw prices before scaling.
>>>>>>> c8fadbe (Add price risk floor and ceiling clamps)
