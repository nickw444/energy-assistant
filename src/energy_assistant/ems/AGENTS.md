...
- `src/energy_assistant/ems/builder.py` (builds the MILP)
- `src/energy_assistant/ems/planner.py` (solves and extracts plan output)
- `src/energy_assistant/ems/horizon.py` (time horizon and slotting)
- `src/energy_assistant/ems/forecast_alignment.py` (forecast alignment)
- `src/energy_assistant/ems/models.py` (typed plan output models; `EmsMilpPlanner.generate_ems_plan` returns `EmsPlanOutput`)
...
- Forecasts resolve through `MILPBuilder.resolve_forecasts(...)` into `ResolvedForecasts`
  (defined in `src/energy_assistant/ems/models.py`) containing `PowerForecastInterval` and
...
Plotting (`src/energy_assistant/plotting/plan.py`):
...
- Record new scenarios via `energy-assistant ems record-scenario --fixture <fixture> --name <scenario>`.
  The config is only written if it doesn't already exist (shared across scenarios).
- Replay fixtures offline via `energy-assistant ems solve --fixture <fixture> --scenario <scenario>`
  to view plots or output JSON without a live Home Assistant connection.
- Build a multi-scenario visual report with `energy-assistant ems scenario-report` to render
...
- When making EMS changes, validate against a checked-in fixture by replaying it
  ...
  Example workflow:
  - `energy-assistant ems solve --fixture nwhass --scenario short-horizon --output /tmp/ems_plan.actual.json`
  - Use `/tmp/ems_plan.actual.json` for deep debugging; the checked-in baseline is summarized for diffs.
- Refresh baselines with `energy-assistant ems refresh-baseline`:
