...
- `src/energy_assistant/ems/system/component.py`, `src/energy_assistant/ems/system/context.py`, `src/energy_assistant/ems/system/state.py`, and `src/energy_assistant/ems/system/types.py` define the typed EMS component contract, build/plan contexts, solve-state store, and component type literals.
...
- `src/energy_assistant/ems/planning/horizon.py` defines the persistent rolling `HorizonFactory` and per-solve `Horizon`.
...
- `AppConfig` (Pydantic) enforces plant wiring invariants: a component's `connection` must name another plant id with the expected component type. `EmsSystemFactory` resolves those string ids to direct `EmsComponent` object references; it raises with a clear error if a key is still missing.
