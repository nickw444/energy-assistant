...
class GridComponent(EmsComponent[GridSolveState, GridComponentPlan]):
    ...
    def create_graph_elements(
        ...
        *,
        ...
        price_import_effective = self._price_binding_applicator.apply(
            horizon=horizon,
            prices=price_import_raw,
            binding=self._config.price_import,
            direction="import",
        )
        price_export_effective = self._price_binding_applicator.apply(
            horizon=horizon,
            prices=price_export_raw,
            binding=self._config.price_export,
            direction="export",
        )
        import_allowed = self._resolve_import_allowed(horizon)
        cfg = self._config
        import_eff = price_import_effective
        export_eff = price_export_effective
