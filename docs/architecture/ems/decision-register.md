...
## D032 — Keep constructor dependency wiring explicit
- Status: accepted and implemented as the intended architecture; remaining violations should be refactored toward this shape
- Decision: require runtime classes to accept their instance collaborators through `__init__` rather than constructing those collaborators internally.
- Why: constructor-time creation hides the real dependency graph, weakens test seams, and reintroduces service-location behavior inside otherwise modular classes.
- Consequence: default production wiring belongs in outer composition code such as `EmsSystemFactory` or in explicit convenience `create(...)` classmethods, while primary constructors remain suitable for direct injection of fakes and mocks.
