"""Wiring is checked in two layers: Pydantic (schema) and :class:`EmsSystemFactory` (resolution)."""
...
def test_pydantic_rejects_incompatible_connection_target_type() -> None:
    """Schema rules: a battery's ``connection`` must name an inverter, not a switchboard."""
    payload = {
...
def test_pydantic_rejects_missing_connection_target() -> None:
    """Schema rules: ``connection`` must name another component in the plant registry."""
    payload = {
        "server": _MINIMAL_SERVER,
        "homeassistant": _MINIMAL_HA,
        "inputs": {
    ...
    with pytest.raises(ValueError, match="missing connection target"):
        AppConfig.model_validate(payload)
...
def test_factory_raises_when_dependencies_cannot_be_resolved() -> None:
    ...
    with pytest.raises(ValueError, match=r"unresolved component connections: \['inverter', 'battery'\]"):
        EmsSystemFactory(
