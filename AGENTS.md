...
- EV charging includes a built-in time preference penalty to bias charging earlier in the horizon.
- EV hysteresis uses a soft on/off switch penalty (no ramp penalty).
- EV SoC incentives are biased with the import premium so they stay comparable to biased import costs.
