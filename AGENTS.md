...
- Battery wear uses `battery.wear` with `mode: symmetric` to apply a single cost per kWh of throughput; the cost is scaled by `ems.max_profit_sacrifice_per_day`.
- `EmsPlanOutput` now includes `objective_value` with the solver objective (may be negative/None).
- Grid export during negative prices is blocked when `grid.allow_negative_export` is false; use load-aware curtailment to avoid forced export.
