...
- Optimizers mirror mappers: configure `optimizer.module` (plus optional `attribute`) and load via `hass_energy.optimizer.load_optimizer`. The protocol `HassEnergyOptimizer` exposes `required_entities()` (knobs/settings pulled from HA) and `decide(mapped, entities)`. `run-optimizer` runs mapper + optimizer once and prints the decision.
- A rule-based optimizer tailored to `mapper.py` lives at the repo root (`optimizer.py`, class `MapperAlignedOptimizer`); point `optimizer.module` to `./optimizer.py:MapperAlignedOptimizer` (or `./optimizer.py:get_optimizer`) to use it.
...
- Websocket client auto-reconnects with backoff on connection loss and re-subscribes existing entity subscriptions; explicit `disconnect()` stops reconnection.
